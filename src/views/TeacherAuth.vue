...
import { setAuthSession, teacherLogin, teacherRegister, teacherSendCode } from '../api/auth'
...
const sendCode = async () => {
  ...
  devCodeHint.value = ''
  try {
    const data = await teacherSendCode(phoneText)
    startCodeTimer(Number(data.expireInSeconds || 60))
    if (data.debugCode) {
      devCodeHint.value = `开发环境验证码：${data.debugCode}`
    }
  } catch (error) {
    errorText.value = (error && error.message) || '验证码发送失败，请稍后重试'
  } finally {
...
const nextStep = () => {
  if (currentStep.value === 1) {
    if (!/^1\d{10}$/.test(phone.value.trim()) || !password.value || !code.value.trim()) {
  ...
  if (currentStep.value === 2) {
    if (!nickname.value.trim() || !subject.value.trim()) {
      errorText.value = '请填写姓名和擅长科目'
      return
    }
    if (teachingMethods.value.length === 0) {
      errorText.value = '请至少选择一种授课方式'
      return
    }
  }
...
const finishRegister = async () => {
  ...
  try {
    await teacherRegister({
      ...
      certType: certUrl.value ? certType.value : undefined,
      certUrl: certUrl.value || undefined
    })
    currentStep.value = 4
  } catch (error) {
...
const handleTeacherLogin = async () => {
  if (!/^1\d{10}$/.test(phone.value.trim()) || !password.value) {
    errorText.value = '请填写正确的手机号和密码'
    return
  }
  loading.value = true
  errorText.value = ''
...
const toggleTeachingMethod = (value) => {
  const index = teachingMethods.value.indexOf(value)
  if (index >= 0) teachingMethods.value.splice(index, 1)
  else teachingMethods.value.push(value)
}
</script>
